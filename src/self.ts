 ...
 * 0.12.1：锚点那句补上「可以用 memory_update 改写或清除」——此前模型读得到锚点的作用，
 * 却没有任何改它的手段，读起来像一条只能写死一次的东西。
 */
...
export const SELF_DESCRIPTION =
  ...
  + '**锚点（可选）**：若这条记忆描述的是「随某个环境值变化的事实」（某工具版本下的行为、某环境变量决定的配置），用 anchor 参数把它绑到那个值上——值变了它会自动失效并标 stale，不会再被当成仍然正确；绑错了用 memory_update 改（当场重新判定，成立即复活），不要了就传 `anchor: null` 清除。当前锚点随记录一起返回。'
